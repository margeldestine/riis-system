-- V2__create_missing_materialized_views.sql
--
-- REPLACES the first version of this file. Enabling Flyway against this
-- live database revealed, incrementally, that multiple pieces of
-- V1__initial_schema.sql were never actually applied here -- not just
-- mv_cluster_distribution (the first thing that failed), but also the
-- clusters and research_output_clusters TABLES it depends on. Rather
-- than keep discovering and patching one missing piece at a time, this
-- migration is a fully idempotent copy of EVERYTHING V1 creates, so it
-- safely fills in whatever is actually missing from this specific
-- database in one pass, regardless of which subset that turns out to be.
--
-- Every statement below is byte-for-byte the same logic as V1, with
-- safety added only where V1 didn't already have it:
--   - CREATE TABLE statements: already IF NOT EXISTS in V1, unchanged.
--   - CREATE INDEX statements: 15 of 17 were already IF NOT EXISTS in
--     V1; the 2 HNSW vector indexes were not, so IF NOT EXISTS was
--     added to those two here.
--   - The 2 ALTER TABLE ... ADD CONSTRAINT statements: Postgres has no
--     ADD CONSTRAINT IF NOT EXISTS syntax, so each is wrapped in a
--     DO block that catches "already exists" (duplicate_object) and
--     does nothing, instead of failing the whole migration.
--   - CREATE MATERIALIZED VIEW statements: already IF NOT EXISTS from
--     this file's first version.
--
-- Nothing here DROPs, DELETEs, or ALTERs the shape of anything that
-- already exists -- every statement either creates something that is
-- missing, or safely confirms it is already there and moves on.
...
-- research_outputs already existed before this migration ever ran, so
-- its CREATE TABLE IF NOT EXISTS above was skipped entirely and never
-- got the chance to add the cluster_id / cluster_assigned_at columns
-- V1 defines. Adding them explicitly here -- both nullable, unused by
-- any current application code, so this is a pure additive schema
-- catch-up with zero behavioral effect on anything working today.
ALTER TABLE research_outputs ADD COLUMN IF NOT EXISTS cluster_id VARCHAR(36);
ALTER TABLE research_outputs ADD COLUMN IF NOT EXISTS cluster_assigned_at TIMESTAMP;
...
CREATE INDEX IF NOT EXISTS idx_research_outputs_specter_embedding_hnsw ON research_outputs USING hnsw (specter_embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);
CREATE INDEX IF NOT EXISTS idx_research_outputs_sbert_embedding_hnsw ON research_outputs USING hnsw (sbert_embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);

-- 9. MATERIALIZED VIEWS
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_analytics_trend AS
SELECT
    ro.institution_id,
    ro.research_type,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_hei_comparison AS
SELECT
    ro.institution_id,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_cluster_distribution AS
SELECT
    c.id AS cluster_id,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_heatmap_matrix AS
SELECT
    tp.institution_id,
    tk.keyword,
