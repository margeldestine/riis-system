...
function StatCard({ value, label, sublabel, borderColor, textColor, onClick }) {
  return (
    <div onClick={onClick} style={{ borderLeft: `4px solid ${borderColor}`, background: '#fff', borderRadius: 10, padding: '20px 24px', flex: 1, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', cursor: onClick ? 'pointer' : 'default' }}>
...
// ---------------------------------------------------------------------------
// RejectAccountModal — SDD §5.6
// ---------------------------------------------------------------------------
function RejectAccountModal({ user, onConfirm, onCancel, loading }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
        ...
        />
        <StatCard
          value={countsLoading ? '…' : (approvedCount ?? '—')}
          label="Approved"
        ...
        <div style={{ width: 320, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#eff6ff', border: '1.5px solid #bfdbfe', borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.7 }}>
              <div style={{ marginBottom: 6 }}>✓ <strong>Approve</strong> — Account status changes to Active. Staff receives email notification and can log in immediately.</div>
...
// ---------------------------------------------------------------------------
// AccountApprovalQueue — SDD §5.6 root component
// ---------------------------------------------------------------------------
export default function AccountApprovalQueue() {
  ...
  const loadAllCounts = async () => {
    ...
    try {
      ...
      console.error('[AccountApprovalQueue] Failed to load counts:', err)
    } finally {
  ...
  const loadPendingUsers = async () => {
    try {
      ...
      console.error('[AccountApprovalQueue] Failed to fetch pending users:', error)
      setPendingUsers([])
    } finally {
      setIsLoading(false)
    }
  ...
  const loadAll = async () => {
    await Promise.all([loadPendingUsers(), loadAllCounts()])
  }
  ...
  const handleApprove = async () => {
    try {
      ...
      console.error('[AccountApprovalQueue] Approve failed:', error)
    } finally {
  ...
  const handleReviewNext = () => {
    const remaining = pendingUsers.filter((u) => u.id !== approvedUser?.id)
  ...
  return (
    <DashboardLayout navItems={dostNavItems} userName="DOST Administrator" organization="DOST Region VII">
      <QueueScreen
        users={pendingUsers}
        onReview={(user) => { setSelectedUser(user); setScreen('review') }}
